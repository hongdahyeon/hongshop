 ...
 *          (2) updateStatus : 배송 상태 정보 수정   ->  배송 상태에 따른 주문 상태도 변경
 *          (3) getByOrderId : 주문 Id로 배송 정보 조회 -> return vo
 *          (4) getHongDeliverByOrderId : 주문 Id로 배송 정보 조회 -> return entity
 *          (5) updateAddress : 배송 주소 정보 수정
 *              -> 배송 정보 수정은 배송 상태가 AWAIT일 경우에만 가능
 *          (6) allWithChkReview : 배송 정보 전체 조회 with review write boolean
**/
...
public class HongDeliverServiceImpl implements HongDeliverService {
    ...
    @Override
    public List<HongDeliverReviewChkVO> allWithChkReview() {
        ...
        return all.stream().map(hongDeliver -> {
            ...
            return new HongDeliverReviewChkVO(hongDeliver, reviewEmpty);
        }).toList();
    }
